            ...
            for tcall in step.step_details.tool_calls:
                if getattr(tcall, "function", None):
                    ...
                    try:
                        ...
                        # For example, if function was "search_jobs"
                        if fn_name == "search_jobs":
                            message = "Found job search results."
                            if self.create_tool_bubble_fn:
                                self.create_tool_bubble_fn(fn_name, message, tcall.id)

                        elif fn_name == "send_job_info_sms":
                            if "message" in output:
                                message = f"SMS sent successfully: {output['message']}"
                            elif "error" in output:
                                message = f"Error sending SMS: {output['error']}"
                            else:
                                message = "No response from SMS."
...
def create_chat_interface(project_client, agent, thread, tracer=None):
    ...
    def azure_job_chat(user_message: str, history: List[dict]):
